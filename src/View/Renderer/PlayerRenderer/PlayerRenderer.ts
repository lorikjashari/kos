...
export abstract class PlayerRenderer implements IUpdatable {
  ...
  protected game: Game
  protected showDebug = import.meta.env.DEV
  ...
  public handleShoot(hitscanResult: HitscanResult): void {
    ...
    if (weapon.fireMode === 'melee') {
      void this.game.audioManager.playKnife()
      if (hitscanResult.hasHit) {
        void this.game.audioManager.playKnifeHit()
      }
    } else if (weapon.playsGunshot) {
      void this.game.audioManager.playShot(weapon.key)
    }

    if (hitscanResult.hitBot && hitscanResult.bodyPart && hitscanResult.hitPosition) {
      const isHead = hitscanResult.bodyPart === 'head'
      void this.game.audioManager.playFleshHit(isHead)
      if (hitscanResult.killed) {
        void this.game.audioManager.playBotDeath()
      }
      const idx = hitscanResult.botIndex
      this.game.renderer.bloodManager?.spawn(
        hitscanResult.hitPosition,
        hitscanResult.hitNormal,
        hitscanResult.bodyPart,
        idx !== undefined && idx >= 0 ? this.game.botRenderers[idx]?.getRoot() : undefined
      )
      this.game.renderer.hud?.showHitMarker(isHead)
      if (idx !== undefined && idx >= 0) {
        this.game.botRenderers[idx]?.flashHit()
      }
    }
    ...
    if (weapon.spawnsProjectile) {
      ...
      this.game.renderer.projectileManager.spawn(origin, direction, undefined, weapon.bulletSpeed)
    }
  ...
  protected getMuzzleOrigin(): Vector3D {
    const eye = this.player.position.clone().add(new Vector3D(0, this.player.eyeOffsetY, 0))
    const direction = this.player.lookingDirection.clone().normalize()
    const right = new Vector3D().crossVectors(new Vector3D(0, 1, 0), direction).normalize()
    return eye
      .add(direction.clone().multiplyScalar(0.9))
      .add(right.multiplyScalar(0.18))
      .add(new Vector3D(0, -0.1, 0))
  }
